...
2) the user follows the link (logs in to https://kauri.finance), choose in which way payment should be done and pay it
...
### Creating invoice
 ...
 ```javascript
 {
  ...
  "payment_option": restrict in which way user should pay invoice. Choices - ALL, FIAT, CRYPTO. Default value - ALL
...
```
 ...
 ALL - User can pay in all avialiable choices
 FIAT - User can pay only from Kauri Finance or via FIAT GATEWAY(login to Kauri Finance not required)
 CRYPTO - User can pay only from Kauri Finance or via CRYPTO GATEWAY(login to Kauri Finance not required)
...
In order to pay, the user needs to follow the specified link, log in to https://kauri.finance, choose the currency and go through the payment process.
